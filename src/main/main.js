...
class PriceListManager {
  ...
  async importFromBuffer(buffer, options = { merge: true }) {
    try {
      ...
      const workbook = XLSX.read(new Uint8Array(buffer), { type: 'array', defval: '', raw: false });
        ...
        try {
          ...
          if (existingItem) {
            if (existingItem.price !== price || existingItem.name !== name) {
              newItems[id] = { id, name, price, updatedAt: new Date().toISOString(), previousPrice: existingItem.price };
              stats.updated++;
            } else {
            ...
            newItems[id] = { id, name, price, createdAt: new Date().toISOString() };
            stats.added++;
          }
        } catch (e) {
          stats.errors++;
      ...
      return { stats, totalItems: Object.keys(this.data.items).length, lastUpdate: this.data.lastUpdate };
    } catch (err) {
  ...
  getAll() {
    return Object.values(this.data.items).sort((a, b) => parseInt(a.id) - parseInt(b.id));
  }
}
